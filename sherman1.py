...
def main():
    ...
    # # x00, it_log00, res_log00, rho00 = solvers.DampedJacobi(A, b, x0, omega=.0, tol=args.t, maxiter=args.i)
    # x02, it_log02, res_log02, rho02 = solvers.DampedJacobi(A, b, x0, omega=.2, tol=args.t, maxiter=args.i)
    # x04, it_log04, res_log04, rho04 = solvers.DampedJacobi(A, b, x0, omega=.4, tol=args.t, maxiter=args.i)
    # x06, it_log06, res_log06, rho06 = solvers.DampedJacobi(A, b, x0, omega=.6, tol=args.t, maxiter=args.i)
    # x08, it_log08, res_log08, rho08 = solvers.DampedJacobi(A, b, x0, omega=.8, tol=args.t, maxiter=args.i)
    # x10, it_log10, res_log10, rho10 = solvers.DampedJacobi(A, b, x0, omega=1., tol=args.t, maxiter=args.i)
    # x12, it_log12, res_log12, rho12 = solvers.DampedJacobi(A, b, x0, omega=1.2, tol=args.t, maxiter=args.i)
    # x14, it_log14, res_log14, rho14 = solvers.DampedJacobi(A, b, x0, omega=1.4, tol=args.t, maxiter=args.i)
    # x16, it_log16, res_log16, rho16 = solvers.DampedJacobi(A, b, x0, omega=1.6, tol=args.t, maxiter=args.i)
    # x18, it_log18, res_log18, rho18 = solvers.DampedJacobi(A, b, x0, omega=1.8, tol=args.t, maxiter=args.i)
    # # x20, it_log20, res_log20, rho20 = solvers.DampedJacobi(A, b, x0, omega=2.0, tol=args.t, maxiter=args.i)
    ...
    # # SOR method
    # # x00, it_log00, res_log00, rho00 = solvers.SOR(A, b, x0, omega=.0, tol=args.t, maxiter=args.i)
    # x02, it_log02, res_log02, rho02 = solvers.SOR(A, b, x0, omega=.2, tol=args.t, maxiter=args.i)
    # x04, it_log04, res_log04, rho04 = solvers.SOR(A, b, x0, omega=.4, tol=args.t, maxiter=args.i)
    # x06, it_log06, res_log06, rho06 = solvers.SOR(A, b, x0, omega=.6, tol=args.t, maxiter=args.i)
    # x08, it_log08, res_log08, rho08 = solvers.SOR(A, b, x0, omega=.8, tol=args.t, maxiter=args.i)
    # x10, it_log10, res_log10, rho10 = solvers.SOR(A, b, x0, omega=1., tol=args.t, maxiter=args.i)
    # x12, it_log12, res_log12, rho12 = solvers.SOR(A, b, x0, omega=1.2, tol=args.t, maxiter=args.i)
    # x14, it_log14, res_log14, rho14 = solvers.SOR(A, b, x0, omega=1.4, tol=args.t, maxiter=args.i)
    # x16, it_log16, res_log16, rho16 = solvers.SOR(A, b, x0, omega=1.6, tol=args.t, maxiter=args.i)
    # x18, it_log18, res_log18, rho18 = solvers.SOR(A, b, x0, omega=1.8, tol=args.t, maxiter=args.i)
    # # x20, it_log20, res_log20, rho20 = solvers.SOR(A, b, x0, omega=2.0, tol=args.t, maxiter=args.i)
    ...
    # condition number
    cond = np.linalg.cond(A, p=2)
    x_j, it_log_j, res_log_j, rho_j = solvers.Jacobi(A, b, x=x0, tol=args.t, maxiter=args.i)
    x_dj, it_log_dj, res_log_dj, rho_dj = solvers.DampedJacobi(A, b, x=x0, omega=.8, tol=args.t, maxiter=args.i)
    x_gs, it_log_gs, res_log_gs, rho_gs = solvers.GaussSeidel(A, b, x=x0, tol=args.t, maxiter=args.i)
    x_sor, it_log_sor, res_log_sor, rho_sor = solvers.SOR(A, b, x=x0, omega=1.8, tol=args.t, maxiter=args.i)
    x_sd, it_log_sd, res_log_sd, alpha_sd = solvers.SteepDesc(A, b, x=x0, tol=args.t, maxiter=args.i)
    x_cg, it_log_cg, res_log_cg, alpha_cg = solvers.ConjGrad(A, b, x=x0, tol=args.t, maxiter=args.i)
